    ...
    <section className="mb-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {items.map((item, index) => (
          <div key={index} className="bg-white p-3 rounded shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
            {item.title && <h4 className="font-medium text-sm mb-1">{item.title}</h4>}
            {item.code && <pre className="text-xs bg-gray-50 p-2 rounded mb-2 overflow-x-auto">{item.code}</pre>}
